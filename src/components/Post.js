...
const Post = () => {
  ...
  useEffect(() => {
    ...
    return (
        <main className="bg-green-100 min-h-screen p-12 w-screen">
                  ...
                  {postData && postData.map((post, index) => (
                    <article>
                      <Link to={"/post/" + post.slug.current} key={post.slug.current}>
                        <span className="block h-64 relative rounded shadow-sm leading snug bg-white border-l-8 border-green-400" key={index}>
